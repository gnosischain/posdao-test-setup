...
module.exports = async function (web3, sendTx, stakingAuRa) {
    // `8` is to account for possible period of snapshotting
    let stakeWithdrawDisallowPeriod = parseInt(await stakingAuRa.methods.stakeWithdrawDisallowPeriod().call());
    ...
    while (currentBlock <= startBlock + maxRetriesBlocks) {
        ...
        if ( !(await stakingAuRa.methods.areStakeAndWithdrawAllowed().call()) ) {
        ...
        try {
            ...
            if (blocksPassed <= stakeWithdrawDisallowPeriod && !!(await stakingAuRa.methods.areStakeAndWithdrawAllowed().call())) {
